...
use crypto::{hashers::Blake2b_256, ElementHasher, MerkleTree};
use math::{
...
fn extend_trace_table() {
    ...
    let trace_tree = trace_lde.commit_to_rows::<Blake2b>();
    let trace_comm = TraceCommitment::<BaseElement, Blake2b>::new(
        trace_lde,
        trace_tree,
...
fn commit_trace_table() {
    ...
    for i in 0..trace_table.trace_len() {
        ...
        let buf = Blake2b::hash_elements(&trace_state);
        hashed_states.push(buf);
    }
    let expected_tree = MerkleTree::<Blake2b>::new(hashed_states).unwrap();
