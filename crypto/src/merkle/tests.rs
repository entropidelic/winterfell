...
type Blake2s_256 = crate::hash::Blake2s_256<BaseElement>;
...
fn new_tree() {
    ...
    let tree = MerkleTree::<Blake2s_256>::new(leaves.clone()).unwrap();
...
fn verify() {
    // depth 4
    let leaves = Digest256::bytes_as_digests(&LEAVES4).to_vec();
    let tree = MerkleTree::<Blake2s_256>::new(leaves).unwrap();
    let proof = tree.prove(1).unwrap();
    assert!(MerkleTree::<Blake2s_256>::verify(*tree.root(), 1, &proof).is_ok());

    let proof = tree.prove(2).unwrap();
    assert!(MerkleTree::<Blake2s_256>::verify(*tree.root(), 2, &proof).is_ok());
    ...
    assert!(MerkleTree::<Blake2s_256>::verify(*tree.root(), 6, &proof).is_ok());
}
...
proptest! {
    ...
    fn prove_n_verify(tree in random_blake3_merkle_tree(128),
        ...
        for proof_index in proof_indices{
            ...
            prop_assert!(MerkleTree::<Blake2s_256>::verify(*tree.root(), proof_index.index(128), &proof).is_ok())
        }
    }
