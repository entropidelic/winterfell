...
use winter_crypto::{
    hashers::{Blake3_256, Blake2s_256, Rp62_248, Rp64_256, Sha3_256},
    Hasher,
};
...
type Blake2s = Blake3_256<f128::BaseElement>;
type Blake2sDigest = <Blake3 as Hasher>::Digest;
...
fn blake2s(c: &mut Criterion) {
    let v: [Blake2sDigest; 2] = [Blake2s::hash(&[1u8]), Blake2s::hash(&[2u8])];
    c.bench_function("hash_blake2s (cached)", |bench| {
        bench.iter(|| Blake2s::merge(black_box(&v)))
    });

    c.bench_function("hash_blake2s (random)", |b| {
        b.iter_batched(
            || {
                [
                    Blake2s::hash(&rand_value::<u64>().to_le_bytes()),
                    Blake2s::hash(&rand_value::<u64>().to_le_bytes()),
                ]
            },
            |state| Blake2s::merge(&state),
            BatchSize::SmallInput,
        )
    });
}
